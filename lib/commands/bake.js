...
const { resolveSource, resolveRepoFlag } = require('../modules/utils/source');
...
exports.builder = (yargs) => {
    yargs
        ...
        .example(`$0 bake ottomatica/baker-test`, `Clone github.com/ottomatica/baker-test into ./baker-test and Bake its top-level baker.yml`)
        .example(`$0 bake your-org/configs:units/one`, `Clone the repo and Bake the baker.yml in its units/one directory`)
        .example(`$0 bake --repo git@github.com:ottomatica/baker-test.git`, `Clone into ./baker-test and Bake its baker.yml`)
        .example(`$0 bake --repo https://github.com/ottomatica/baker-examples/tree/master/jenkins`, `Clone into ./baker-examples and Bake the baker.yml in its jenkins subdirectory`)
...
exports.handler = async function(argv) {
    ...
    try{
        ...
        if (local) {
            ...
            bakePath = path.resolve(await resolveRepoFlag(repo));
        } else if (file) {
