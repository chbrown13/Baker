...
class Platform {
    ...
    // Whether install commands need a `sudo` prefix on this target.
    //
    // A Docker container runs as root and usually has no sudo binary at all, so
    // prefixing unconditionally would break docker-local — which used to work,
    // because Ansible's `become: yes` is a no-op when you are already root.
    // Assumes sudo is needed when the answer cannot be read, since that is the
    // safe direction: an unnecessary sudo on a root shell still works.
    static async needsSudo(execCapture) {
        try {
            const uid = String(await execCapture('id -u')).trim();
            return uid !== '0';
        } catch (err) {
            return true;
        }
    ...
    static async detect(execCapture, platformName = process.platform) {
        ...
        if (platformName === 'win32') {
            // Chocolatey needs an elevated shell, never a sudo prefix.
            return { os: 'windows', manager: 'choco', shell: 'powershell', family: 'nt', sudo: false };
        }
        if (platformName === 'darwin') {
            // Homebrew refuses to run under sudo.
            return { os: 'macos', manager: 'brew', shell: 'sh', family: 'darwin', sudo: false };
        }
        ...
        return {
            os: 'linux', manager, shell: 'sh', family: Platform.familyOf(manager),
            sudo: await Platform.needsSudo(execCapture)
        };
    }
}
