...
class Baker {
    ...
    static async list() {
        ...
        await new LocalProvider().list();
        await new DockerLocalProvider().list();
    }
    ...
    static async chooseProvider(bakePath, useContainer, useVM){
        ...
        let envType = doc.docker ? 'docker-local' : doc.local ? 'local' : doc.container || doc.persistent ? 'container' : doc.vm || doc.vagrant ? 'vm' : doc.remote ? 'remote' : 'other';
        ...
        else if(envType === 'local')
        {
            ...
            console.error('This command only supports VM, container, docker, and local environments');
