...
class LocalProvider extends Provider {
    ...
    // named "~" under the current working directory. Utils.expandTilde is the
    // single implementation `files:`, the bakelet base, and remote key paths
    // all share, so every place a ~ can appear in a config agrees.
    static resolveLocation(location) {
        return path.resolve(Utils.expandTilde(location));
    }
    ...
    async bake(scriptPath, ansibleSSHConfig, verbose) {
        ...
        // Record the environment, as the docker-local provider already does.
        // Without this a local bake leaves no trace at all — no index entry and
        // no `.running` marker, since start() has had no caller since `up` and
        // `halt` were removed — so nothing downstream can tell a baked project
        // from an unbaked one. `baker run` needs exactly that distinction.
        //
        // The path recorded is the resolved LOCATION rather than scriptPath:
        // with `local: <path>` the two differ, and the location is where the
        // bake actually acted.
        // Added by Claude Code (claude-opus-5[1m])
        await Utils.addToIndex(doc.name, location, 'local', {});
