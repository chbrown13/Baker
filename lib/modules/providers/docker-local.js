...
class DockerLocalProvider extends Provider {
    ...
    async list() {
        try {
            let envIndex = await Utils.getEnvIndex();
            let dockerEntries = envIndex.filter(e => e.type === 'docker-local');
            let table = [];
            for (let entry of dockerEntries) {
                try {
                    let container = this.docker.getContainer(entry.info.id);
                    let info = await container.inspect();
                    table.push({
                        name: entry.name,
                    ...
                    table.push({
                        name: entry.name,
                        state: 'removed',
                        image: entry.info.image || 'N/A'
                    });
                }
            }
            console.table('\nBaker Docker containers: ', table);
        } catch (err) {
            console.error('Unable to list Docker containers: ' + err.message);
        }
    }
