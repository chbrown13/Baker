...
class Env extends Bakelet {
    ...
    // Removing env: is surgical — the profile keeps every line the user wrote.
    // On POSIX, Baker's variables live in their own file, so the inverse is to
    // delete that file and drop the one source line the profile gained. On
    // Windows the variables are registry entries, cleared one by one.
    async plan() {
        if (!this.envVars.length) {
            return [{ kind: 'none', bakelet: 'env', reason: 'no variables declared' }];
        }
        // POSIX keeps Baker's variables in one file; if it is gone there is
        // nothing left to undo, which is what makes a second cleanup a no-op.
        if (this.shell !== 'powershell' && !(await this.filterExisting(['~/.baker/env.sh'])).length) {
            return [{ kind: 'none', bakelet: 'env', reason: '~/.baker/env.sh already gone' }];
        }
        if (this.shell === 'powershell') {
            return [{
                kind: 'exec', bakelet: 'env', default: true,
                command: this.envVars
                    .map((v) => `[Environment]::SetEnvironmentVariable("${v.key}", $null, "User")`)
                    .join('; '),
                summary: `unset ${this.envVars.length} user environment variable(s)`,
                restore: 'baker bake <same source>'
            }];
        }
        return [{
            kind: 'exec', bakelet: 'env', default: true,
            command: 'rm -f ~/.baker/env.sh && ' +
                'sed -i "/baker\\/env.sh/d" ~/.profile 2>/dev/null || true',
            summary: `remove ~/.baker/env.sh and its ~/.profile source line ` +
                `(${this.envVars.length} variable(s))`,
            restore: 'baker bake <same source>'
        }];
    }
