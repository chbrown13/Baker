...
class Files extends Bakelet {
    ...
    resolveDest(dest) {
        if (Files.isAbsoluteish(dest)) {
            ...
            const expanded = this.isLocal && dest.startsWith('~')
    ...
    // Sources are relative to the baker.yml directory. The traversal guard
    // deliberately does NOT apply here: overlays legitimately climb out with
    // ../../base/, and the source side is instructor-authored.
    resolveSrc(src) {
        return path.resolve(this.bakePath, src);
    }
