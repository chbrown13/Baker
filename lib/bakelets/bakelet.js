...
class Bakelet
{
    ...
    //
    // Shares Utils.expandTilde with the providers rather than reimplementing
    // it. The old slice(1) here also rewrote `~user/x` to <home>user/x, which
    // named a path nobody asked for; expandTilde leaves that form alone.
    resolveLocalPath(p) {
        return Utils.expandTilde(p);
    }
    ...
    // `ref` is optional and only affects the restore hint: a clone made with
    // --branch is not restored by a plain `git clone`, so the command handed
    // back has to name the ref that produced it.
    // Modified by Claude Code (claude-opus-5[1m])
    async planRepoRemoval(label, dest, url, ref) {
        if (!dest) {
        ...
        return {
            ...
            restore: url ? `git clone ${branch}${url} ${dest}` : `git clone ${branch}<url> ${dest}`
        };
    }
    ...
    // Removes a directory only if nothing is left in it. Used where Baker
    // created a directory but does not own everything that ended up inside:
    // files: overlays, and resources: git: extract:, which injects into a folder
    // the person already owns.
    // Moved up from Files by Claude Code (claude-opus-5[1m]) so both share one
    // implementation.
    async removeIfEmpty(full) {
        if (this.localLocation) {
            try {
                const remaining = await fs.readdir(this.resolveLocalPath(full));
                if (!remaining.length) await fs.remove(this.resolveLocalPath(full));
