...
module.exports.resolveBakelet = async function(bakeletsPath, remotesPath, doc, bakerScriptPath, verbose, localLocation, remoteSSHConfig, dockerContainer)
{
    ...
    try {
        ...
        if( doc.lang )
        {
            ...
            {
                await resolve(doc.name, bakerScriptPath, remotesPath, path.join(bakeletsPath,"lang"), doc.lang[i], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
            }
        }
        ...
        if( doc.config )
        {
            ...
            {
                await resolve(doc.name, bakerScriptPath, remotesPath, path.join(bakeletsPath,"config"), doc.config[i], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
            }
        }
        ...
        if( doc.services )
        {
            ...
            {
                await resolve(doc.name, bakerScriptPath, remotesPath, path.join(bakeletsPath,"services"), doc.services[i], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
            }
        }
        ...
        if( doc.tools )
        {
            ...
            {
                await resolve(doc.name, bakerScriptPath, remotesPath, path.join(bakeletsPath,"tools"), doc.tools[i], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
            }
        }
        ...
        if( doc.packages )
        {
            ...
            {
                await resolve(doc.name, bakerScriptPath, remotesPath, path.join(bakeletsPath,"packages"), doc.packages[i], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
            }
        }
        ...
        if( doc.resources )
        {
            ...
            {
                await resolve(doc.name, bakerScriptPath, remotesPath, path.join(bakeletsPath,"resources"), doc.resources[i], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
            }
        }
        ...
        {
            doc.env = [{env: doc.env}];
            await resolve(doc.name, bakerScriptPath, remotesPath, path.join(bakeletsPath,"env"), doc.env[0], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
        }
        ...
        if( doc.custom )
        {
            for (var i = 0; i < doc.custom.length; i++)
            {
                ...
                await resolveCustom(doc.name, bakerScriptPath, bakerScriptPath, doc.custom[i][info.bakeletName].path, info, doc.custom[i], extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer);
            }
        }
...
async function resolveCustom(vmName, bakerScriptPath, remotesPath, bakeletPath, info, bakelet, extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer) {
    const localMode = !!localLocation;
    const remoteMode = !!remoteSSHConfig;
    const dockerMode = !!dockerContainer;
    let mod = info.mod;
    ...
    if (localMode) {
        j.copy = async (src, dest) => {
            const localDest = path.join(localLocation, path.basename(dest));
            await fs.copy(src, localDest);
        };
        j.exec = async (cmd) => {
            let adjustedCmd = cmd.replace(
                new RegExp(`/home/vagrant/baker/${vmName}/`, 'g'),
                localLocation + '/'
            );
            child_process.execSync(adjustedCmd, {cwd: localLocation, encoding: 'utf8'});
        };
        j.setLocalLocation(localLocation);
    } else if (remoteMode) {
        j.copy = async (src, dest) => {
            await Ssh.copyFromHostToVM(src, dest, remoteSSHConfig, false);
        };
        j.exec = async (cmd) => {
            await Ssh.sshExec(cmd, remoteSSHConfig, 20000, verbose);
        };
    } else if (dockerMode) {
        j.copy = async (src, dest) => {
        ...
        j.exec = async (cmd) => {
            child_process.execSync(`docker exec "${dockerContainer}" /bin/bash -c '${cmd}'`, {
...
async function resolve(vmName, bakerScriptPath, remotesPath, dir, bakelet, extra_vars, verbose, localLocation, remoteSSHConfig, dockerContainer) {
    const localMode = !!localLocation;
    const remoteMode = !!remoteSSHConfig;
    const dockerMode = !!dockerContainer;
