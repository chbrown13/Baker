...
class Latex extends PackageTool {
    ...
    // Mirrors the install list exactly. TeX Live is large, so this is one of
    // the few inverses people actually want.
    // Added by Claude Code (claude-opus-5[1m])
    get uninstallCommands() {
        return {
            apt: `${this.sudo}apt-get remove -y texlive-latex-base texlive-latex-extra texlive-latex-recommended texlive-fonts-recommended latexmk`,
            dnf: `${this.sudo}dnf remove -y texlive-scheme-basic texlive-latexmk`,
            pacman: `${this.sudo}pacman -Rns --noconfirm texlive-core texlive-latexextra`,
            zypper: `${this.sudo}zypper --non-interactive remove texlive-latex texlive-latexmk`,
            apk: `${this.sudo}apk del texlive texmf-dist-latexextra`,
            brew: 'brew uninstall --cask basictex',
            choco: 'choco uninstall -y miktex'
        };
    }
}
