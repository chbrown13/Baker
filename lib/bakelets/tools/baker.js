...
class Baker extends PackageTool {
    ...
    // The installed package is `baker` whatever shorthand `source:` used, so the
    // inverse does not need the source at all — which is just as well, since a
    // cleanup may run against a config that has since changed it.
    // Added by Claude Code (claude-opus-5[1m])
    get uninstallCommands() {
        const npm = 'npm uninstall -g baker';
        return {
            apt: npm, dnf: npm, pacman: npm, zypper: npm, apk: npm, brew: npm, choco: npm
        };
    }

    // Removing the Baker that is running the cleanup would work — the process
    // is already in memory — but every later `baker` invocation fails. Worth
    // saying before someone selects it.
    get removalPrompt() {
        return `Remove baker? This uninstalls Baker itself; the current cleanup ` +
            `finishes, but later baker commands will not run.`;
    }
}
