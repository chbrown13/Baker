...
class AgenticTool extends Bakelet {
    ...
    async plan() {
        ...
        out.push(...await this.planConfigDirRemoval(repoDest));
        return out;
    }

    // The tool's OWN config directory — ~/.config/opencode, ~/.claude.
    //
    // Baker does not create it; the tool does, on first run. Uninstalling the
    // binary and leaving it behind means a cleaned machine still holds the
    // tool's settings and whatever credentials it saved, which is the opposite
    // of what someone running cleanup asked for. So it is offered — but as the
    // most cautious kind of offer this bakelet makes.
    //
    // default: false, and the prompt says plainly that Baker did not create it.
    // This is a deliberate exception to "Baker does not delete what it did not
    // create": the directory only exists because Baker installed the tool that
    // made it, and nothing else will ever clean it up.
    //
    // Skipped entirely when a config repo: was cloned to the same path, because
    // planRepoRemoval already covers it and applies the dirty-clone guard that
    // a blind rm -rf here would bypass.
    // Added by Claude Code (claude-opus-5[1m])
    async planConfigDirRemoval(repoDest) {
        const dir = this.defaultConfigDir;
        if (!dir) return [];

        if (repoDest && this.samePath(repoDest, dir)) return [];

        if (!(await this.filterExisting([dir])).length) {
            return [{
                kind: 'none', bakelet: `${this.toolKey} config dir`,
                reason: `${dir} not present`
            }];
        }

        return [{
            kind: 'paths', bakelet: `${this.toolKey} config dir`, default: false,
            paths: [dir],
            prompt: `Remove ${dir}? This is ${this.binName}'s own settings directory, ` +
                `not something Baker created — it may hold your saved login and preferences.`,
            summary: `remove ${dir}`,
            restore: `recreated by ${this.binName} on next run; settings are not recoverable`
        }];
    }

    // Compares two config paths without touching the filesystem, so planning
    // stays side-effect free. Both sides are home-relative in practice, so
    // expanding ~ is enough to make them comparable.
    samePath(a, b) {
        return path.resolve(this.resolveLocalPath(a)) === path.resolve(this.resolveLocalPath(b));
    }
