...
class Cpp extends PackageTool {
    ...
    // build-essential and base-devel are metapackages: removing them takes gcc
    // and make with them, which a lot of unrelated software needs to build.
    // Added by Claude Code (claude-opus-5[1m])
    get uninstallCommands() {
        return {
            apt: `${this.sudo}apt-get remove -y build-essential`,
            dnf: `${this.sudo}dnf remove -y gcc-c++ make`,
            pacman: `${this.sudo}pacman -Rns --noconfirm base-devel`,
            zypper: `${this.sudo}zypper --non-interactive remove gcc-c++ make`,
            apk: `${this.sudo}apk del build-base`,
            brew: 'brew uninstall gcc',
            choco: 'choco uninstall -y mingw'
        };
    }

    get removalPrompt() {
        return `Remove the C++ toolchain? This is a metapackage on most systems — ` +
            `gcc and make go with it, and other software may depend on them.`;
    }
}
